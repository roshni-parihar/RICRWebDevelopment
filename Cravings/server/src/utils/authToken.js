...
export const genToken = async (user, res) => {
  try {
    const payload = {     // payload is a actual data that we want to encrpt or making a token so, that it shouldn't be accessed by 3rd party
      id: user._id,
    ...
    res.cookie("parleG",token,{ // here that jwt is saved in cookie and send it over to ..frontend.. secretly
