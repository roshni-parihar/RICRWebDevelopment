...
const UserSideBar = ({ active, setActive, isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <div>
        <button className="flex  gap-3  items-center  p-3 rounded-xl text-base h-12 w-full  text-nowrap overflow-hidden duration-300  text-white hover:bg-red-200" onClick={handleLogout}> <MdLogout />{!isOpen && "Logout"}
        </button>
      </div>
      </div>
    </>
